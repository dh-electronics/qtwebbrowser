    ...
    Rectangle {
        ...
        states: [
            ...
            State {
                ...
                PropertyChanges {
                    target: navigation
                    state: GlobalSettings.kioskmodeEnabled?"disabled":"enabled"
                }
            },
            State {
                name: "tabsfull"
                PropertyChanges {
                    target: message
                    text: qsTr("%1 open tabs is the maximum limit.\nTo open a new tab you must close another one first.").arg(GlobalSettings.maxTabCount)
                }
                PropertyChanges {
                    target: messageBox
                    visible: true
